...
async createTask(tasktype_immoblier:string, taskbuyer:string, taskseller:string, tasprice:string){
   
      this.Contract.methods.createTask(tasktype_immoblier, taskbuyer, taskseller, tasprice).send(
        {
          from: this.account,
          gas: '6721975'
        }
  ...
  async getAll(){
    ...
    let getTask=this.Contract.methods.getAll().call(
      {
        from:this.account,
        gas: '6721975',
        gasPrice: '20000000',
